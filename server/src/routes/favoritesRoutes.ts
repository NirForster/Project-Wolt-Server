...
router.get("/", getUserFavoritesShops); // Send: 200, 400, 404, 500 ({ message?: string, status: "Success" | "Error",  favoritesShops?: Shop[] })

//* Add new shop to the user favorites
//! PUT http://localhost:3000/api/v1/favorites/add
router.put("/add", addToFavorites); // Send: 200, 400, 404, 500 ({ message: string, status: "Success" | "Error" })

//* Add new shop to the user favorites
//! PUT http://localhost:3000/api/v1/favorites/remove
router.put("/remove", removeFromFavorites); // Send: 200, 400, 404, 500 ({ message: string, status: "Success" | "Error" })
