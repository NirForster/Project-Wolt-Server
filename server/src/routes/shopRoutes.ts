...
router.post("/:id/review", userAuth, addNewReview); // Send: 200, 404, 500 ({ message: string, status: "Success" | "Error" })

//* Get the last order the user made from this shop
//! GET http://localhost:3000/api/v1/shop/:id/last-order
router.get("/:id/last-order", userAuth, getShopLastOrder); //Send: 200, 204, 400, 404, 500 ({message?: string, status?: "Success" | "Error", order? :})
...
router.get("/:id", getShopData); // Send: 201, 400, 403, 404 500 ({ message: string, status: "Success" | "Error" })
